...
public static class ApiKeyResolver
{
    ...
    public static (string PartitionKey, ApiKeyTier Tier) Resolve(HttpContext context)
    {
        var key = context.Request.Headers["X-Api-Key"].ToString();

        if (string.IsNullOrEmpty(key))
            return (context.Connection.RemoteIpAddress?.ToString() ?? "anonymous", ApiKeyTier.Anonymous);
